...
class PYCEncryptor(object):
    def __init__(self):
        ...
        self.pyc27_header = b"\x03\xf3\x0d\x0a\x00\x00\x00\x00"

    def _decrypt_file(self, filename):
        content = open(filename, "rb").read()
        try:
            m = pymarshal.loads(content[8:])
        except Exception:
            try:
                m = marshal.loads(content[8:])
            except Exception as exc:
                print("[!] error: %s" % str(exc))
                return None
        pyc_filename = getattr(m, "co_filename", "output.py")
        return str(pyc_filename).replace("\\", "/"), pymarshal.dumps(m, self.opcode_encrypt_map)
    ...
    def decrypt_file(self, input_file, output_file=None):
        ...
        if not output_file:
            output_file = os.path.basename(pyc_filename) + ".out"
